...
int generateBytecode(const ExpressionNode* node, std::vector<RawInstruction>& outCode, std::vector<double>& pool, int& regTracker, std::map<std::string, int>& vMap) {
    ...
    case NodeType::BinaryOp: {
        ...
        if (cmd == CommandID::BR_EQ || cmd == CommandID::BR_LT || cmd == CommandID::BR_GT) {
            ...
            outCode.push_back({ (uint32_t)cmd, (uint32_t)leftReg, (uint32_t)rightReg, 2 });

            
            outCode.push_back({ (uint32_t)CommandID::SET_VAL, (uint32_t)resultReg, (uint32_t)pool.size(), 0 });
            pool.push_back(0.0); 

            
            outCode.push_back({ (uint32_t)CommandID::BR_EQ, (uint32_t)leftReg, (uint32_t)leftReg, 2 }); 
            ...
            outCode.push_back({ (uint32_t)cmd, (uint32_t)resultReg, (uint32_t)leftReg, (int32_t)rightReg });
        }
        return resultReg;
    }
