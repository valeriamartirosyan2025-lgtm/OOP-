...
void runScanner(const std::string& input) {
    ...
    while (i < len) {
        ...
        if (isdigit(input[i])) {
            std::string val = "";
            while (i < len && (isdigit(input[i]) || input[i] == '.')) { val += input[i++]; }
            tokenStream.push_back({ TokenKind::NUM, val });
        }
        else if (isalpha(input[i])) {
            std::string word = "";
            while (i < len && isalnum(input[i])) { word += input[i++]; }
            ...
            if (s == "+" || s == "-" || s == "*" || s == "/") tokenStream.push_back({ TokenKind::OP, s });
            else if (s == "=" && i + 1 < len && input[i + 1] == '=') { s += "="; i++; tokenStream.push_back({ TokenKind::OP, s }); }
            else if (s == "<" || s == ">") tokenStream.push_back({ TokenKind::OP, s });
...
TokenUnit& activeToken() { return tokenStream[streamPos]; }
bool isEndOfStream() { return streamPos >= (int)tokenStream.size(); }
TokenUnit pullToken() { return tokenStream[streamPos++]; }